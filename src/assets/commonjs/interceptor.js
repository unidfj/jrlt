...
axios.interceptors.request.use(
    ...
    function(config){
        if(process.env.NODE_ENV == 'development'){
            // if(config.url=='http://www.unionchinapay.com/Pay_Index.html'){
            //     config.url = config.url;
            // }else{
            //     config.url = process.env.proxy+config.url+'?token='+localStorage.getItem('cookie')+'&lang='+localStorage.getItem('lang');
            // }
            config.url = process.env.proxy+config.url+'?token='+localStorage.getItem('cookie')+'&lang='+localStorage.getItem('lang');
        }else {
            // if(config.url=='http://www.unionchinapay.com/Pay_Index.html'){
            //     config.url = config.url;
            // }else{
            //     config.url = process.env.API+process.env.PORT+config.url+'?token='+localStorage.getItem('cookie')+'&lang='+localStorage.getItem('lang');
            // }
            config.url = process.env.API+process.env.PORT+config.url+'?token='+localStorage.getItem('cookie')+'&lang='+localStorage.getItem('lang');
        }
        return config;
