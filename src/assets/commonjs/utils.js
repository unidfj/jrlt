...
export const Util = new class {
	...
	pageToImg(vue,callback,dom='.body'){
		vue.html2canvas(document.querySelector(dom),{useCORS: true,}).then(canvas=>{
...
     */
	 ...
	 */
	isWhite(route){
		let whiteRoutes = ['Login','Register','BackPwd','Download','Welcome','UploadWgt','openView','Close','Close2','Pay','Pay2','Pay3','Capture']
...
export const HB = new class {
                    ...
                    plus.io.resolveLocalFileSystemURL(e, function(entry) { 
						entry.file(function(file) {
							...
							reader.onloadend = function(e) {
								...
								formData.append('image',dataBase);
	 ...
	 * @param {文件} fileName 
	 * @param {图片id} imgID 
	 * @param {是否覆盖原文件} overwrite 
	 * @param {*保存的格式} format 
	 * @param {图片质量} quality 
	 * @param {裁剪的定点} clip 
	 */
	saveScreen(successCB,errorCB,fileName=Date.parse(new Date()),imgID=String(Date.parse( new Date())),overwrite=true,format='png',quality='50',clip={top:'0px',left:'0px',width:'100%',height:'100%'}){
        ...
        try{
            ...
            self.draw(bitmap,function(){
				console.log('_doc/'+fileName+'.'+format)
				// 保存Bitmap图片
...
// 						case '1':
// 							_this.sendShare(String(e.index),'WXSceneSession',imgSrc);
// 							break;
// 						case '2':
// 							_this.sendShare(String(e.index),'WXSceneSession',imgSrc);
// 							break;
// 						case '3':
// 							_this.sendShare(0,'WXSceneSession',imgSrc);
// 							break;
// 					}
// 				});
//             }catch(e){}
...
// 			console.log(index,scene,imgSrc)
...
export const Share = new class {
	...
	sendShare(index,scene,imgSrc,bl,title){
		...
		let img = "https://ftp.bmp.ovh/imgs/2020/04/93aa0cdc8264eef5.png";
